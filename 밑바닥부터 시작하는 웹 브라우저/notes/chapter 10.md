...
실습 예제의 댓글 입력 폼에 스크립트를 삽입하여 서비스에 침투시키고 다른 사용자의 실행 흐름에 해당 스크립트가 실행되도록해서 다양한 공격을 시도할 수 있다.

이를 방지하기 위한 서비스 레벨에서의 표준 해법은 데이터를 코드로 해석할 수 없도록 인코딩하는 것이다.

웹 브라우저 입장에서는 이와 같은 취약점을 방지하기 위해 Content-Security-Policy 헤더를 처리할 수 있어야 한다.
...
- Content-Security-Policy 헤더 처리 구현
  - Content-Security-Policy 헤더의 전체 사양은 상당히 복잡하지만 실습 브라우저에서는 default-src 키워드만 구현해본다.
  - Tab 객체의 load 메서드에서 페이지를 로드할 때 응답 헤더의 Content-Security-Policy를 추출하여 파싱해놓는다. 올바른 CSP 처리를 위해서 자바스크립트나 CSS에 대한 로드를 하기 전에 선행해야 한다.
  - 스크립트와 CSS 파일을 로드하는 과정에 CSP 설정 여부 확인 및 검증 과정을 추가한다.