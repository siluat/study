...
- 러스트의 클로저는 변수에 저장하거나 다른 함수에 인수로 전달할 수 있는 익명 함수다. 한 곳에서 클로저를 만들고 다른 컨텍스트의 다른 곳에서 이를 호출하여 평가할 수 있다. 함수와 다르게 정의된 스코프에서 값을 캡처할 수 있다.
- 클로저는 세 가지 방식(불변으로 빌려오기, 가변으로 빌려오기, 소유권 이동)으로 자신의 환경으로부터 값을 캡처할 수 있다. 함수가 매개변수를 취하는 세 가지 방식과 직접 대응된다. 클로저는 캡처된 값이 쓰이는 방식에 기초하여 캡처할 방법을 결정할 것이다.
- `move` 키워드를 사용하여 클로저로 소유권을 이동시킬 수 있다. 대체로 클로저를 새 스레드에 넘길 때 유용하다.
- 클로저는 클로저의 본문이 값을 처리하는 방식에 따라서 다음 `Fn` 트레이트 중 하나, 둘, 혹은 셋 모두를 추가한 방식으로 **자동**으로 구현할 것이다.
  - `FnOnce`는 한 번만 호출될 수 있는 클로저에게 적용된다.모든 클로저들은 호출될 수 있으므로, 최소한 이 트레이트는 구현해 둔다. 캡처된 값을 본문 밖으로 이동시키는 클로저는 딱 한 번만 호출될 수 있기 때문에 `FnOnce`만 구현되며 나머지 `Fn` 트레이트는 구현되지 않는다.
  - `FnMut`은 본문 밖으로 캡처된 값일 이동시키지 않지만 값을 변경할 수 있는 클로저에 적용된다. 이런 클로저는 한 번 이상 호출될 수 있다.
  - `Fn`은 캡처된 값을 본문 밖으로 이동시키지 않고 캡처된 값을 변경하지도 않는 클로저는 물론, 환경으로부터 아무런 값도 캡처하지 않는 클로저에 적용된다. 이런 클로저는 자신의 환경을 변경시키지 않으면서 한번 이상 호출될 수 있는데, 이는 클로저가 동시에 여러 번 호출되는 등의 경우에서 중요하다.

# Iterators

- 반복자를 소비하는 메서드를 소비 어댑터(consuming adaptor)라고 한다. (e.g. `sum` 메서드, `let total: i32 = vec![1, 2, 3].iter().sum()`)
- 원본 반복자의 어떤 측면을 바꿔서 다른 반복자를 제공하는 메서드를 반복자 어댑터(iterator adaptor)라고 한다. (e.g. `map` 메서드, `let v2: Vec<_> = vec![1, 2, 3].iter().map(|x| x + 1).collect()`)
